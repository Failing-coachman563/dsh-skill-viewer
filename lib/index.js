...
const batchMigrateResultSchema = z.object({
  results: z.array(z.object({ name: z.string(), ok: z.boolean(), error: z.string().optional() }))
});
...
class SkillsViewerGateway extends TypertRemoteService {
  ...
  /** Migrate a batch of skills between two scope folders; per-item results. */
  async batchMigrate(sessionId, payload) {
    const { from: rawFrom, target: rawTarget, mode, names } = payload;
    const { dshHome, agentsHome } = this.homes();
    ...
    const targetRoot = scopeRootOf(targetProject, dshHome);
    if (fromRoots.some((root) => resolvePath(root.path) === resolvePath(targetRoot))) throw new Error("源作用域与目标作用域相同");
    ...
    results.push(...(await batchMigrateEntries(chosen, targetRoot, mode)));
    return { results };
  }
